...
age_data = {
    '나이대': ['0-9', '10-19', '20-29', '30-39', '40-49', '50-59', '60-69', '70-79', '80-'],
    '총 확진자': [5147974, 7508796, 10947729, 11041128, 11614230, 10220411, 8229533, 4300724, 2518002]
}
...
sns.barplot(data=age_df, x='나이대', y='총 확진자', palette='viridis')
plt.title('나이대별 총 확진자 수')
plt.xlabel('나이대')
plt.ylabel('총 확진자 수')
...
# 나이대별 확진자 분석
total_cases = age_df['총 확진자'].sum()
age_df['비율(%)'] = (age_df['총 확진자'] / total_cases) * 100
print("나이대별 확진자 비율 (%)")
print(age_df[['나이대', '총 확진자', '비율(%)']])
